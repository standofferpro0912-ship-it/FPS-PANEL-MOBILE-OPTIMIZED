...
local function GetAimbotTarget(camera)
    ...
    local bestPlayer, bestPart, bestMetric
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and (not Config.TeamCheck or not IsTeammate(player)) then
            ...
            if character and humanoid and humanoid.Health > 0 and targetPart and root then
                local distance = localRoot and (root.Position - localRoot.Position).Magnitude or math.huge
                if distance <= Config.MaxAimDistance then
                    local screen, onScreen = camera:WorldToViewportPoint(targetPart.Position)
                    if onScreen and screen.Z > 0 then
                        local screenPos = Vector2.new(screen.X, screen.Y)
                        local cursorDistance = (screenPos - screenCenter).Magnitude
                        if cursorDistance <= Config.AimFOV and IsTargetVisible(camera, targetPart, character) then
                            local metric
...
local function Aimbot(_dt)
    ...
    -- Critical behavior:
    -- We ONLY write to the camera while a valid visible target exists.
    -- When the target disappears behind a wall / leaves FOV / dies,
    -- we do NOTHING to Camera.CFrame. That leaves the camera at the
    -- exact orientation where the target was last acquired, while the
    -- player remains completely free to move the camera normally.
    local _, targetPart = GetAimbotTarget(camera)
    ...
    local offset = aimPoint - camera.CFrame.Position
    ...
    -- Instant snap. No smooth interpolation and no return-to-previous-view.
