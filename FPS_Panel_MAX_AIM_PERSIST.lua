...
local particleStateBackup={}
local lowGraphicsApplied=false
local reduceParticlesApplied=false

local function IsEffectObject(object)
    return object and (object:IsA("ParticleEmitter") or object:IsA("Trail") or object:IsA("Beam"))
end

local function SetEffectSuppressed(object,suppress)
    if not IsEffectObject(object) then return end
    if suppress then
        if particleStateBackup[object]==nil then
            particleStateBackup[object]=object.Enabled
        end
        object.Enabled=false
    else
        local old=particleStateBackup[object]
        if old~=nil and object.Parent then
            object.Enabled=old
        end
        particleStateBackup[object]=nil
    end
end

local function RefreshEffectSuppression()
    local suppress=Config.LowGraphics or Config.ReduceParticles
    if suppress then
        for _,object in ipairs(workspace:GetDescendants()) do
            if IsEffectObject(object) then
                SetEffectSuppressed(object,true)
            end
        end
        lowGraphicsApplied=Config.LowGraphics
        reduceParticlesApplied=Config.ReduceParticles
    elseif next(particleStateBackup)~=nil then
        for object,enabled in pairs(particleStateBackup) do
            if object and object.Parent then
                object.Enabled=enabled
            end
        end
        table.clear(particleStateBackup)
...
pcall(function()
    if Env.FPSPanelDescendantConnection then
        Env.FPSPanelDescendantConnection:Disconnect()
    end
end)
Env.FPSPanelDescendantConnection=workspace.DescendantAdded:Connect(function(object)
    if (Config.LowGraphics or Config.ReduceParticles) and IsEffectObject(object) then
        task.defer(function()
            if object and object.Parent and (Config.LowGraphics or Config.ReduceParticles) then
                SetEffectSuppressed(object,true)
            end
        end)
    end
...
local function ApplyLowGraphics()
    ...
    if Config.LowGraphics then
        ...
        if not lowGraphicsApplied then
            RefreshEffectSuppression()
        end
    else
        if terrain and TerrainBackup.WaveSize~=nil then
        ...
        if reduceParticlesApplied then
            -- ReduceParticles is still active; keep shared suppression in place.
            lowGraphicsApplied=false
        else
            RefreshEffectSuppression()
        end
