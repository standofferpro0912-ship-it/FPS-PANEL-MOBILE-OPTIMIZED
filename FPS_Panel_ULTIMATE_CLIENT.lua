...
local function Toggle(p,title,desc,get,set)
    ...
    sw.Activated:Connect(function() set(not get()); refresh() end)
...
Toggle(Pages.Combat,"Team Check","Ignore players on the same team when possible.",function() return Config.TeamCheck end,function(v) Config.TeamCheck=v end)
...
local function ClosestTarget()
    ...
    local center=cam.ViewportSize/2; local best=nil; local bestDist=Config.MaxAimDistance
    for _,p in ipairs(Players:GetPlayers()) do
        if p~=LocalPlayer and p.Character then
            if Config.TeamCheck and LocalPlayer.Team ~= nil and p.Team == LocalPlayer.Team then
                continue
            end
            local tc=p.Character; local h=tc:FindFirstChildOfClass("Humanoid"); local head=tc:FindFirstChild("Head"); local tr=tc:FindFirstChild("HumanoidRootPart")
            if h and h.Health>0 and head and tr then
                local wd=(tr.Position-root.Position).Magnitude
                if wd<=Config.MaxAimDistance then
                    local sp,on=cam:WorldToViewportPoint(head.Position)
                    if on and sp.Z>0 then
                        local sd=(Vector2.new(sp.X,sp.Y)-center).Magnitude
                        if sd<=Config.AimFOV and wd<bestDist and VisiblePart(head) then bestDist=wd; best=head end
                    end
                end
            end
        end
...
local function Aimbot(dt)
    ...
    local cam = workspace.CurrentCamera
    local head = ClosestTarget()

    if cam and head then
        local target = CFrame.lookAt(cam.CFrame.Position, head.Position)
